...
// clang-format off
const uint64_t PolyglotBook::Random64[781] = {
  // Pieces: [piece][square] - 12 piece types x 64 squares
  // Polyglot piece order: bp=0, wp=1, bn=2, wn=3, bb=4, wb=5, br=6, wr=7, bq=8, wq=9, bk=10, wk=11
  // Random64[piece*64 + square]
...
bool PolyglotBook::load(const std::string& filename) {
  entries.clear();

  std::ifstream file(filename, std::ios::binary);
  if (!file.is_open()) {
    Logger::getInstance().warning("Could not open Polyglot book: " + filename);
    return false;
  }
  ...
  while (file.good()) {
    ...
    entry.move = (uint16_t(bytes[8]) << 8) | uint16_t(bytes[9]);
    entry.weight = (uint16_t(bytes[10]) << 8) | uint16_t(bytes[11]);
...
uint64_t PolyglotBook::computeHash(const Position& pos) {
  ...
  // Pieces
  // Polyglot piece order: wP=0, bP=1, wN=2, bN=3, wB=4, bB=5, wR=6, bR=7, wQ=8,
  // bQ=9, wK=10, bK=11
  for (Square sq = A1; sq <= H8; sq = Square(sq + 1)) {
    ...
    if (p != NO_PIECE) {
      ...
      if (polyPiece >= 0) {
        // Polyglot uses rank-major order: index = piece*64 + rank*8 + file
        int polySquare = rankOf(sq) * 8 + fileOf(sq);
        hash ^= Random64[polyPiece * 64 + polySquare];
      }
    }
  }
  ...
  if (ep != NO_SQUARE) {
    // Check if there's actually an enemy pawn that can capture
    int epFile = fileOf(ep);
    bool canCapture = false;

    if (pos.sideToMove() == WHITE) {
      // White to move, check for white pawns on rank 5 adjacent to ep file
      if (epFile > 0) {
        Square leftPawn = Square((RANK_5 * 8) + epFile - 1);
        if (pos.pieceAt(leftPawn) == W_PAWN) canCapture = true;
      }
      if (epFile < 7) {
        Square rightPawn = Square((RANK_5 * 8) + epFile + 1);
        if (pos.pieceAt(rightPawn) == W_PAWN) canCapture = true;
      }
    } else {
      // Black to move, check for black pawns on rank 4 adjacent to ep file
      if (epFile > 0) {
        Square leftPawn = Square((RANK_4 * 8) + epFile - 1);
        if (pos.pieceAt(leftPawn) == B_PAWN) canCapture = true;
      }
      if (epFile < 7) {
        Square rightPawn = Square((RANK_4 * 8) + epFile + 1);
        if (pos.pieceAt(rightPawn) == B_PAWN) canCapture = true;
      }
    }

    if (canCapture) {
      hash ^= RandomEnPassant[epFile];
    }
  }

  // Side to move (Polyglot: white=0, black=hash with RandomTurn)
  if (pos.sideToMove() == WHITE) {
    hash ^= RandomTurn;
  }
...
Move PolyglotBook::convertMove(uint16_t polyMove, const Position& pos) const {
  ...
  // Handle castling: Polyglot encodes as king captures own rook
  Piece piece = pos.pieceAt(from);
  if (typeOf(piece) == KING) {
    // Check for castling
    if (fromFile == FILE_E && toFile == FILE_H) {
      // Kingside castling
      to = Square(toRank * 8 + FILE_G);
    } else if (fromFile == FILE_E && toFile == FILE_A) {
      // Queenside castling
      to = Square(toRank * 8 + FILE_C);
    }
  }
  ...
  for (Move m : legalMoves) {
    if (fromSquare(m) == from && toSquare(m) == to) {
      ...
      if (promo > 0) {
        ...
        PieceType promoType = promotionType(m);
        // Polyglot: 1=knight, 2=bishop, 3=rook, 4=queen
        PieceType expectedPromo = NO_PIECE_TYPE;
        switch (promo) {
          case 1:
            expectedPromo = KNIGHT;
            break;
          case 2:
            expectedPromo = BISHOP;
            break;
          case 3:
            expectedPromo = ROOK;
            break;
        ...
        if (promoType != expectedPromo) continue;
      }
      return m;
    }
  }
...
Move PolyglotBook::probe(const Position& pos) const {
  ...
  auto [start, end] = findEntries(key);

  if (start == end) return 0;  // No entries found
  ...
  for (size_t i = start; i < end; ++i) {
    Move m = convertMove(entries[i].move, pos);
    if (m != 0) {
...
std::vector<std::pair<Move, uint16_t>> PolyglotBook::getMoves(
  ...
  for (size_t i = start; i < end; ++i) {
    Move m = convertMove(entries[i].move, pos);
    if (m != 0) {
      result.push_back({m, entries[i].weight});
    }
  }
