...
Bitboard relevantOccupancy(Square sq, const int directions[4]) {
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    while (true) {
      ...
      bool atEdge = false;
      if (dir == 8 && rankOf(to) == RANK_8) atEdge = true;   // Going north, reached rank 8
      if (dir == -8 && rankOf(to) == RANK_1) atEdge = true;  // Going south, reached rank 1
      if (dir == 1 && fileOf(to) == FILE_H) atEdge = true;   // Going east, reached file H
      if (dir == -1 && fileOf(to) == FILE_A) atEdge = true;  // Going west, reached file A
      if (dir == 9 && (rankOf(to) == RANK_8 || fileOf(to) == FILE_H)) atEdge = true;   // NE edge
      if (dir == 7 && (rankOf(to) == RANK_8 || fileOf(to) == FILE_A)) atEdge = true;   // NW edge
      if (dir == -7 && (rankOf(to) == RANK_1 || fileOf(to) == FILE_H)) atEdge = true;  // SW edge
      if (dir == -9 && (rankOf(to) == RANK_1 || fileOf(to) == FILE_A)) atEdge = true;  // SE edge
...
void init() {
  ...
  for (Square sq = A1; sq <= H8; ++sq) {
    ...
    for (int i = 0; i < (1 << n); ++i) {
      ...
      RookMagics[sq].attacks[index] = slidingAttacks(sq, occupied, RookDirections);
    }
  ...
  for (Square sq = A1; sq <= H8; ++sq) {
    ...
    for (int i = 0; i < (1 << n); ++i) {
      ...
      BishopMagics[sq].attacks[index] = slidingAttacks(sq, occupied, BishopDirections);
    }
  ...
  for (Square sq1 = A1; sq1 <= H8; ++sq1) {
    for (Square sq2 = A1; sq2 <= H8; ++sq2) {
      ...
      if (rookAttack & sq2BB) {
        LineTable[sq1][sq2] =
            (rookAttacks(sq1, BB::EMPTY) & rookAttacks(sq2, BB::EMPTY)) | sq1BB | sq2BB;
        BetweenTable[sq1][sq2] = rookAttacks(sq1, sq2BB) & rookAttacks(sq2, sq1BB);
      } else if (bishopAttack & sq2BB) {
        LineTable[sq1][sq2] =
            (bishopAttacks(sq1, BB::EMPTY) & bishopAttacks(sq2, BB::EMPTY)) | sq1BB | sq2BB;
        BetweenTable[sq1][sq2] = bishopAttacks(sq1, sq2BB) & bishopAttacks(sq2, sq1BB);
      } else {
